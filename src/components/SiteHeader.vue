<script lang="ts" setup>
  ...
  import { realmId } from '@/config/rv.config';
  ...
  const realmLogo = defineAsyncComponent(() => import(`@/assets/logos/${realmId}Logo.vue`));
...
<template>
  <header
    :class="viewportStore.isLarge ? '' : 'wrap'"
    class="site-header flex axis1-between axis2-center gap-2 border-b border-gray-light p-1"
  >
    <a
      ...
      >
        ...
        >
          <BasicButtonIcon
            ...
            class="tertiary absolute top-0 right-0 mt-1 mr-1 ml-auto"
      ...
      >
        <div class="search-bar relative mb-2">
          ...
          <BasicButtonIcon
            ...
            @click="handleSearchBarTriggerClick"
            aria-label="Search"
            class="search-bar-submit primary tier-1 absolute top-0 right-0 mt-1/2 mr-1/2 p-1/2"
            title="Search"
          />
        </div>
